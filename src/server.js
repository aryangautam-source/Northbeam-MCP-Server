...
// Factory function: creates a fresh McpServer instance per connection.
// Required for StreamableHTTPServerTransport since each session needs its own server instance.
  ...
  server.tool(
    ...
    async ({ metric_name, start_date, end_date, dimensions }) => {
      try {
        ...
        return {
          content: [{ type: "text", text: `Error fetching metric ${metric_name}: ${error.message}` }],
          isError: true
        };
      }
    },
  ...
  server.tool(
    ...
    async () => {
      try {
        ...
        return {
          content: [{ type: "text", text: JSON.stringify(dimensions, null, 2) }]
        };
      } catch (error) {
        return {
          content: [{ type: "text", text: `Error listing dimensions: ${error.message}` }],
          isError: true
        };
      }
    },
  ...
  server.tool(
    ...
    async ({ start_date, end_date, metrics }) => {
      try {
        ...
        return {
          content: [{ type: "text", text: `Error fetching channel performance: ${error.message}` }],
          isError: true
        };
      }
    },
  ...
  server.tool(
    ...
    async ({ cohort_type, start_date, end_date, metrics }) => {
      try {
        ...
        return {
          content: [{ type: "text", text: `Error fetching cohort analysis: ${error.message}` }],
          isError: true
        };
      }
    },
  ...
  server.tool(
    ...
    async ({ model, start_date, end_date, metrics }) => {
      try {
        ...
        return {
          content: [{ type: "text", text: `Error fetching attribution data: ${error.message}` }],
          isError: true
        };
      }
    },
